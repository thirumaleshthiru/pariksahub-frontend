...
export default function SubTopicAndExam() {
  const params = useParams();
  const router = useRouter();
  const examTypeName = params.examTypeName as string;
  ...
  const [currentQuestionForNormal, setCurrentQuestionForNormal] = useState<any>(null);
  ...
  const fetchQuestions = async () => {
    try {
      ...
      setDataLoaded(false);
      setQuestions([]);
      const response = await axiosInstance.get(`/api/questions/exam/${examTypeName}/subtopic/${subtopicName}`);
  ...
  const getCorrectAnswerOption = (questionItem: any) => {
    return questionItem.options.find((option: any) => 
...
Focus only on helping students understand the concept and reasoning.`;
  ...
  const handleNormalExplanation = (questionItem: any) => {
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="flex justify-center items-center min-h-screen">
          <div className="text-lg text-[#192A41]">No questions available for this combination.</div>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 mt-4">
        ...
        <nav className="mb-4 text-sm text-gray-500">
          ...
          <Link href={`/practice/${topicName}`} className="hover:text-[#C0A063] cursor-pointer">{displayTopicName}</Link>
          <span className="mx-2">/</span>
